    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-5xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-semibold text-gray-900">{t('dashboard.my_projects')}</h2>
          <button
        ...
        {/* New project form */}
        {showForm && (
          <form onSubmit={handleCreate} className="bg-white border border-indigo-200 rounded-xl p-4 mb-4 shadow-sm">
            <input
              autoFocus
              value={newName}
        ...
        {loading ? (
          ...
          <div className="text-center py-16">
            <p className="text-gray-400">{t('dashboard.no_projects')}</p>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {projects.map((project) => (
              <div
                ...
                className="relative bg-white border border-gray-200 rounded-xl p-5 text-left hover:border-indigo-300 hover:shadow-sm transition-all group cursor-pointer"
              >
                <button
                  onClick={(e) => handleDelete(e, project)}
                  className="absolute top-3 right-3 opacity-0 group-hover:opacity-100 text-gray-300 hover:text-red-500 transition-all text-sm p-1 rounded"
                  title={t('dashboard.delete_title')}
                >
                  ✕
                </button>
                <div className="w-8 h-8 bg-indigo-100 group-hover:bg-indigo-200 rounded-lg mb-3 flex items-center justify-center transition-colors">
