  ...
  if (avatarUrl) {
    return (
      <img
        src={avatarUrl}
        alt={name}
        className="w-9 h-9 rounded-full object-cover border border-gray-200 dark:border-gray-600 shrink-0"
      />
    )
  }
  return (
    <div className="w-9 h-9 rounded-full bg-indigo-100 dark:bg-indigo-900/40 text-indigo-700 dark:text-indigo-300 flex items-center justify-center text-sm font-semibold shrink-0">
      {initials}
    </div>
  )
}
...
export default function MembersPage() {
  ...
  const inputCls = "border border-gray-300 dark:border-gray-600 rounded-lg px-3 py-2 text-sm bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 placeholder-gray-400 dark:placeholder-gray-500 focus:outline-none focus:ring-2 focus:ring-indigo-500"
      ...
      <header className="bg-white dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700 shrink-0">
        <div className="px-4 sm:px-6 py-3 flex items-center justify-between gap-2">
          <div className="flex items-center gap-2 sm:gap-3 min-w-0">
            ...
            <h1 className="font-semibold text-gray-900 dark:text-gray-100 truncate">{t('members.page_title')}</h1>
          </div>
          <div className="flex items-center gap-2">
            ...
            <nav className="hidden sm:flex gap-1">
              ...
              <span className="text-sm font-medium text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-950 px-3 py-1.5 rounded-lg">{t('nav.members')}</span>
        ...
        <nav className="flex sm:hidden gap-1 px-4 pb-2">
          ...
          <span className="text-xs font-medium text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-950 px-2.5 py-1.5 rounded-lg">{t('nav.members')}</span>
      ...
      <main className="flex-1 max-w-2xl mx-auto w-full px-4 sm:px-6 py-6 sm:py-8 space-y-6">
        ...
        {isOwner && (
          <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-5">
            <h2 className="font-semibold text-gray-900 dark:text-gray-100 mb-4">{t('members.add_title')}</h2>
            <form onSubmit={handleSearch} className="flex gap-2 mb-3">
              ...
              <button
                type="submit"
                disabled={searching}
                className="bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-300 text-sm font-medium px-4 py-2 rounded-lg transition-colors disabled:opacity-50"
              >
            ...
            {searchError && (
              <p className="text-sm text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-950/50 border border-red-200 dark:border-red-800 rounded-lg px-3 py-2 mb-3">{searchError}</p>
            )}

            {searchResult && (
              <div className="flex flex-col sm:flex-row sm:items-center justify-between bg-indigo-50 dark:bg-indigo-950/50 border border-indigo-200 dark:border-indigo-700 rounded-xl px-4 py-3 gap-3">
                <div className="flex items-center gap-3">
                  <Avatar name={searchResult.full_name} avatarUrl={searchResult.avatar_url} />
                  <div>
                    <p className="text-sm font-medium text-gray-900 dark:text-gray-100">{searchResult.full_name}</p>
                ...
                <div className="flex items-center gap-2">
                  <select
                    ...
                    className="text-sm border border-gray-300 dark:border-gray-600 rounded-lg px-2 py-1.5 bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100 focus:outline-none focus:ring-2 focus:ring-indigo-500"
                  >
        ...
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl overflow-hidden">
          <div className="px-5 py-4 border-b border-gray-100 dark:border-gray-700">
            <h2 className="font-semibold text-gray-900 dark:text-gray-100">
              {t('members.count_label')} <span className="text-gray-400 dark:text-gray-500 font-normal text-sm">({allMembers.length})</span>
            </h2>
          </div>

          <ul className="divide-y divide-gray-100 dark:divide-gray-700">
            {allMembers.map(({ user, role, isOwner: memberIsOwner, member }) => (
              <li key={user.id} className="flex items-center justify-between px-5 py-3 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition-colors">
                <div className="flex items-center gap-3">
                ...
                <div className="flex items-center gap-2">
                  {isOwner && !memberIsOwner && member ? (
                    <select
                      ...
                      className={`text-xs font-medium px-2.5 py-1 rounded-full border border-transparent hover:border-gray-300 dark:hover:border-gray-600 cursor-pointer transition-colors ${ROLE_STYLES[role]} focus:outline-none focus:ring-2 focus:ring-indigo-500`}
                    >
                  ...
                  {isOwner && !memberIsOwner && member && (
                    <button
                      ...
                      className="text-gray-300 dark:text-gray-600 hover:text-red-500 dark:hover:text-red-400 transition-colors text-sm p-1 rounded"
        ...
        {isOwner && <div className="bg-gray-50 dark:bg-gray-800/50 border border-gray-200 dark:border-gray-700 rounded-xl p-4">
          <p className="text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide mb-2">{t('members.roles_legend')}</p>
