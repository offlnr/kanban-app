...
function MiniBar({ value, total, color }: { value: number; total: number; color?: string }) {
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <span className="text-xs text-gray-500 dark:text-gray-400 w-5 text-right">{value}</span>
    </div>
  )
}
...
export default function ProjectStatsPanel({ project, columns, tasks, workPackages, membersCount, onClose }: Props) {
  ...
  return (
    <aside className="fixed inset-y-0 right-0 z-20 w-80 shadow-2xl border-l border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800 overflow-y-auto flex flex-col lg:static lg:w-72 lg:shadow-none">
      <div className="px-5 py-4 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between sticky top-0 bg-white dark:bg-gray-800 z-10">
        <h2 className="font-semibold text-gray-900 dark:text-gray-100 text-sm">{t('stats_panel.title')}</h2>
        <button onClick={onClose} className="text-gray-400 dark:text-gray-500 hover:text-gray-600 dark:hover:text-gray-300 text-lg leading-none" title={t('stats_panel.close')}>✕</button>
      </div>

      <div className="px-5 py-4 space-y-6 flex-1">
        <section>
          <p className="font-medium text-gray-900 dark:text-gray-100 text-sm mb-1">{project.name}</p>
          {project.description && <p className="text-xs text-gray-400 dark:text-gray-500 line-clamp-3">{project.description}</p>}
        </section>

        <section>
          <p className="text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wide mb-2">{t('stats_panel.section_general')}</p>
        ...
        {tasks.length > 0 && lastColumn && (
          <section>
            <p className="text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wide mb-3">{t('stats_panel.section_progress')}</p>
            <div className="flex items-center gap-3 mb-1">
              <div className="flex-1 bg-gray-100 dark:bg-gray-700 rounded-full h-2 overflow-hidden">
                <div className="h-full bg-emerald-500 rounded-full transition-all" style={{ width: `${completionPct}%` }} />
              </div>
              <span className="text-sm font-semibold text-gray-900 dark:text-gray-100 w-10 text-right">{completionPct}%</span>
            </div>
        ...
        {columns.length > 0 && tasks.length > 0 && (
          <section>
            <p className="text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wide mb-3">{t('stats_panel.section_by_col')}</p>
                ...
                return (
                  <div key={col.id}>
                    <div className="flex items-center gap-1.5 mb-1">
                      ...
                      <span className="text-xs text-gray-600 dark:text-gray-300 truncate max-w-36">{col.name}</span>
                    </div>
        ...
        {tasks.length > 0 && (
          <section>
            <p className="text-xs font-semibold text-gray-400 dark:text-gray-500 uppercase tracking-wide mb-3">{t('stats_panel.section_by_priority')}</p>
            <div className="space-y-2">
              {priorityCounts.map(({ priority, count }) => (
                <div key={priority}>
                  <div className="flex items-center gap-1.5 mb-1">
                    <span className={`w-2 h-2 rounded-full shrink-0 ${PRIORITY_COLORS[priority]}`} />
                    <span className="text-xs text-gray-600 dark:text-gray-300">{t(`priority.${priority}`)}</span>
                  </div>
        ...
        {tasks.length === 0 && (
          <div className="text-center py-6">
            ...
            <p className="text-sm text-gray-400 dark:text-gray-500">{t('stats_panel.no_tasks')}</p>
          </div>
        )}
      </div>
    </aside>
